...
// Imprime la bannière en dégradé vertical magenta -> cyan (néon)
func printGradientBanner() {
	left := "  "
	start := [3]int{255, 80, 200} // magenta néon
	end := [3]int{0, 255, 255}    // cyan néon
	for i, line := range bannerLines {
		...
		time.Sleep(12 * time.Millisecond) // léger “draw” progressif
	}
}

// Petit scintillement néon (flicker)
func neonFlicker(c1, c2 string, repeats int, hold time.Duration) {
	for i := 0; i < repeats; i++ {
		...
		clear()
		fmt.Print("\033[1m") // bold
		for _, line := range bannerLines {
			fmt.Println(c2 + "  " + line + "\033[0m")
		}
		time.Sleep(hold)
	}
	fmt.Print("\033[22m") // normal
}
...
// Appel unique au démarrage : effet néon moderne → spinner → progress → clear
